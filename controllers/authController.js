...
class authController {
    ...
    postsignup(req, res) {
        ...
        authSchema.findOne({ username: username })
            .then(data => {
                if (data) { res.render('auth_signup', { layout: false, messageerror: 'Tên tài khoản đã tồn tại, vui lòng dùng tên tài khoản khác' }); }
                else {
                    if (username != 'home' || username != 'messenger' || username != 'search' || username != 'auth' || username != 'uploads') {
                        authSchema.create({
